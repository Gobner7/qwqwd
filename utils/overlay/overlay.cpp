...
//void DisplayInstanceTree(antagonist::roblox::instance_t instance)
//{
				...
				if (draw)
				{
					...
					if (tab == 1)
					{
						...
						ImGui::BeginChild(XorStr("VisualChildWindow"), ImVec2(580, 358), true);
						{
							...
							ImGui::Separator();
							ImGui::Checkbox(XorStr("Health Info"), &globals::healthinfo);
					...
					if (tab == 3)
					{
						...
						ImGui::BeginChild(XorStr("LuaEnvWindow"), ImVec2(580, 358), true, ImGuiWindowFlags_MenuBar);
						{
							antagonist::lua_overlay::render();
						}
						ImGui::EndChild();
					...
					if (ButtonCenteredOnLine(XorStr("Lua")))
					{
						tab = 3;
					}
